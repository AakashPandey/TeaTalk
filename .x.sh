...
banner(){
	...
	tput sgr0
	tput setaf 15 && printf "\n%s\n" " X"  | awk '{ z = '$(tput cols)' - length; y = int(z / 2); x = z - y; printf "%*s%s%*s\n", x, "", $0, y, ""; }' 
	tput sgr0
	printf "\n"
}
...
send(){
	...
	echo $usern: $msg | nc $theirip 31312 
...
listen(){
    ...
    nc -l -p 31313 >> /tmp/.y.txt
    out="$(tail -n 1 /tmp/.y.txt)"
    tput setaf 3 && printf "\n\t\t\t$out"
    printf "\n"
    stty echo
...
BYE(){
	...
	chkBYE="$(tail -n 1 /tmp/.y.txt | awk '{print $2}')"
  	if [ "$chkBYE" == "left" ]; then
  		a=1
	fi
...
while [ "$msg" != "BYE" ]; do 											
	...
	take		
	        ...
	        if [ "$msg" == "BYE" ]; then
	              echo $usern left the chat | nc $theirip 31312 
	              printf "\n"
